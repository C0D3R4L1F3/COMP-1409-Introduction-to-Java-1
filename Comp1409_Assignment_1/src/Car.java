...
public class Car {
	...
	private static int defaultMinutesParked = 0; /*
													 * BONUS for not allowing there
													 * to be a NUMBER for this
													 * variable in my coding
													 */
	...
	private double odometerInKm; /*
									 * BONUS as the name and data type was not
									 * given for this variable on the guidelines
									 */
	...
	public void setLicensePlate(String proposedLicense) {
		...
		} else if (proposedLicense.length() >= MINIMUM_PLATE_LENGTH && proposedLicense
				.length() <= MAXIMUM_PLATE_LENGTH) /*
													 * BONUS for easy to read
													 * code
													 */ {
			licensePlate = proposedLicense;
		} else {
	...
	public void setTransmissionType(char mOrA) {
		if (mOrA == MANUAL_TRANSMISSION) {
			...
			System.out.println("Installation failure:" + " " + ("'") + (mOrA) + ("'") + " "
					+ "is not a valid transmission type."); /*
															 * BONUS for easy to
															 * read code
															 */
		} else {
	...
	public void setMinutesParked(int aMinutesParked) {
		if (aMinutesParked >= defaultMinutesParked) {
			...
			return; // do not store if it is a negative number /*BONUS for
					// figuring out how to Javadoc that it will not accept a
					// negative number */
		}
	}
}
